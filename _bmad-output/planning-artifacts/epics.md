...
**方法论前提**：AI 编程、页面即契约，开发最小单元是页面；V1 本质 = 逐页把原型 mock 替换为「真实 AI 能力 + 多用户持久化」，页面形态基本不动。原型代码（`prototype/app/`）为契约的最终事实基准。
...
- **FR9**：自由探索接入真实 Explorer Agent，连续只读对话记录（不折叠不拆分）；右侧「故事线索」区可直接编辑；「给我一些方向」只把选择写入输入框，不代答、不替用户决定故事。
- **FR10**：自由探索的「整理为故事设定」首屏不可用，须用户补足信息并主动结束探索后才开放。
...
- **UX-DR5 · 自由探索交互契约**：只读连续对话记录不折叠不拆分；右侧线索区直接编辑；「给我一些方向」只写入输入框不代答、勾选为会话级持续偏好直到主动取消；「整理为故事设定」须补足信息 + 主动结束探索后才开放。
...
| FR9 | Epic 2 | 自由探索真实对话 + 线索区可编辑 + 「给我方向」只写入不代答 |
| FR10 | Epic 2 | 自由探索「整理为故事设定」须补足信息+主动结束后才开放 |
...
**Epic 总数：6** ｜ **FR 覆盖：FR1–FR26 全覆盖，无遗漏、无重复** ｜ **自然依赖：E1→E2→E3→E4→E5→E6 顺序推进，每个 epic 独立可验证（E1 登录管作品 / E2 探索存线索 / E3 出设定圣经 / E4 写章定稿=核心闭环 / E5 归档防跑偏 / E6 通读全本）**
...
**Story 依赖**：2.1 →2.2 →{2.3 →2.4 →2.5（引导链）, 2.6 →2.7（自由链）}；两链在 2.2 后并行、互不向前依赖。
**按需建表**：2.2 `exploration_session` · 2.4 `exploration_message` · 2.6 `story_clue`。
...
**Given** 边界说明
**When** 界定 V1 开放条件
**Then** V1 用「≥1 条用户消息 + 主动点按钮」近似；真正的覆盖度/置信度门禁属 EXP-P02（V2），不在本 story
